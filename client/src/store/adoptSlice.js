...
export const unadoptPet = createAsyncThunk(
    ...
    async(petIndex,thunkAPI)=>{
        const { contract, address} = thunkAPI.getState().adoptReducer;
        const result = await contract.methods.unadopt(petIndex).send({from: address});
        // console.log("after adopt result = ",result);
...
const adoptSlice = createSlice({
  ...
  extraReducers: {
      [initWeb3.fulfilled]: (state,action)=>{
        ...
        //   console.log("In fullfil = ",action.payload);
      ...
      [adoptPet.fulfilled]: (state,action)=>{
          ...
          alert("Pet adobted successfully! Have fun");
      },
      [unadoptPet.fulfilled]: (state,action) => {
        // console.log("Unadopted pet fullfile action = ",action);
        if(state.adopters[action.payload.petIndex] === state.address)
        {
